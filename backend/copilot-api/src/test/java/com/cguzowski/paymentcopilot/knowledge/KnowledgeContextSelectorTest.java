...
class KnowledgeContextSelectorTest {
    ...
    void selectsBoundedRunbookAndPolicyContextWithoutFillingFromWeakerTypes() {
        ...
        assertThat(selected.stream()
                        .filter(item -> item.candidate().documentType() == KnowledgeDocumentType.RUNBOOK))
                .hasSize(4);
        assertThat(selected.stream()
                        .filter(item -> item.candidate().documentType() == KnowledgeDocumentType.POLICY))
                .hasSize(3);
        assertThat(selected).extracting(SelectedKnowledgeChunk::selectedPosition)
                .containsExactly(1, 2, 3, 4, 5, 6, 7);
        assertThat(selected).extracting(item -> item.candidate().chunkId())
                .containsExactly(
    ...
    void returnsFewerWhenOneApprovedTypeHasNoEligibleCandidates() {
        List<KnowledgeSearchCandidate> candidates = List.of(
                candidate(KnowledgeDocumentType.RUNBOOK, 1, 0.9),
                candidate(KnowledgeDocumentType.RUNBOOK, 2, 0.8));

        List<SelectedKnowledgeChunk> selected = new KnowledgeContextSelector(4, 3).select(candidates);

        assertThat(selected).hasSize(2);
        assertThat(selected).allSatisfy(item ->
                assertThat(item.candidate().documentType()).isEqualTo(KnowledgeDocumentType.RUNBOOK));
    }

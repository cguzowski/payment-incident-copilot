...
class PostgresKnowledgeRetrievalRepository implements KnowledgeRetrievalRepository {
    ...
    @Override
    public Optional<KnowledgeRetrievalContext> findContext(UUID tenantId, UUID investigationId) {
        return jdbcClient.sql("""
                        ...
                        LEFT JOIN LATERAL (
                            SELECT evidence.id, evidence.status
                            FROM evidence_collection_attempt evidence
                            WHERE evidence.tenant_id = investigation.tenant_id
                              AND evidence.investigation_id = investigation.id
                              AND evidence.source_tool = 'getRecentServiceErrors'
                            ORDER BY evidence.requested_at DESC, evidence.id DESC
                            LIMIT 1
                        ) latest ON TRUE
                        LEFT JOIN LATERAL (
                            ...
                            WHERE evidence.tenant_id = investigation.tenant_id
                              AND evidence.investigation_id = investigation.id
                              AND evidence.source_tool = 'getRecentServiceErrors'
                              AND evidence.status IN ('AVAILABLE', 'PARTIAL')
                            ORDER BY evidence.requested_at DESC, evidence.id DESC
                            LIMIT 1
                        ) applicable ON TRUE
    ...
    public List<KnowledgeRetrievalAttempt> findAll(UUID tenantId, UUID investigationId) {
        List<KnowledgeRetrievalAttempt> attempts = jdbcClient.sql("""
    ...
    private KnowledgeRetrievalContext mapContext(ResultSet resultSet, int rowNumber) throws SQLException {
        UUID latestId = resultSet.getObject("latest_evidence_id", UUID.class);
        UUID applicableId = resultSet.getObject("applicable_evidence_id", UUID.class);
        String latestStatus = resultSet.getString("latest_evidence_status");
        String applicableJson = resultSet.getString("applicable_evidence_content");
        KnowledgeEvidenceReference evidence = latestId == null && applicableId == null
                ? null
                : new KnowledgeEvidenceReference(
                        latestId,
                        latestStatus == null ? null : EvidenceCollectionStatus.valueOf(latestStatus),
                        applicableId,
                        applicableJson == null ? null : readEvidence(applicableJson));
    ...
    private KnowledgeRetrievalAttempt mapAttemptWithoutResults(ResultSet resultSet, int rowNumber) throws SQLException {
        ...
        List<UUID> evidenceIds = evidenceArray == null
                ? List.of()
                : Arrays.asList((UUID[]) evidenceArray.getArray());
    ...
    private static KnowledgeRetrievalAttempt withResults(
            KnowledgeRetrievalAttempt attempt,
    ...
    private ServiceErrorEvidenceContent readEvidence(String json) {
        try {
            return jsonMapper.readValue(json, ServiceErrorEvidenceContent.class);
        } catch (JacksonException exception) {
    ...
    private static String uuidArrayLiteral(List<UUID> identifiers) {
        return identifiers.stream().map(UUID::toString).reduce((left, right) -> left + "," + right)
                .map(joined -> "{" + joined + "}")
                .orElse("{}");
    }

...
class McpServiceErrorEvidenceGateway implements ServiceErrorEvidenceGateway {
    ...
    public EvidenceSourceResult collect(EvidenceCollectionContext context, UUID toolCallId) {
        try {
            return validate(client.call(context, toolCallId), context.correlationId(), toolCallId);
    ...
    private static EvidenceSourceResult validate(
        ...
        boolean contentRequired = status == EvidenceCollectionStatus.AVAILABLE
    ...
    private static void requireExactFields(
        ...
        if (!values.keySet().containsAll(required)
                || !union(required, optional).containsAll(values.keySet())) {
            throw new InvalidEvidenceSourceResultException();
        }
    }
    ...
    private static final class InvalidEvidenceSourceResultException extends RuntimeException {
    }
}

...
Status: Complete — accepted after Docker-backed zero-skip verification
Created: 2026-08-31
...
- [x] `SynTenPdfCatalogImportServiceTest.validatesTheWholeCorpusBeforeOneAtomicWrite`
...
- [x] Flyway V9 preserves V1-V8 checksums and enforces source-format/hash,
      ...
      rows.
- [x] `SynTenPdfCatalogPostgresIntegrationTest.catalogsThirtyVersionsWithStablePdfLocators`
...
- [x] Hybrid-search integration proves an approved unembedded PDF chunk is
...
- [x] Retrieval persistence and HTTP tests prove source format, safe filename,
...
- [x] Focused backend/frontend suites, PostgreSQL integration tests, Spotless,
...
- 2026-08-31: Implemented fail-closed manifest/source validation, PDFBox page
  ...
  all 30 PDFs into 705 chunks with 27 approved and 3 superseded versions. The
  exact count was corrected during K4 after its first strict planner assertion
  exposed that the original K3 test had checked only a lower bound.
...
- 2026-08-31: Restored Docker and executed the previously blocked PostgreSQL
  contracts. The rollback test first exposed Spring translating the intentional
  changed-version `IllegalArgumentException`; declaring that exception on the
  proxied repository boundary preserved the contract, and the regression then
  passed before the complete focused database group was rerun.
- 2026-08-31: The scoped backend, frontend, and repository gates passed,
  followed by the authoritative zero-skip gate. K3 is accepted; live PDF
  embedding and retrieval evaluation remain the separate K4 objective.
...
- The focused PostgreSQL 17.11 group passed 11/11 tests with zero skips,
  covering Flyway V1-V9, catalog idempotency and rollback, lexical/vector
  eligibility, tenant filtering, retrieval persistence, and the retrieval API.
- `./verify.ps1 -Scope Backend` passed 210 copilot API tests and 9 operations
  MCP tests with zero failures, errors, or skips, including Spotless and both
  deployable JAR builds.
- The authoritative `./verify.ps1` gate passed 210 copilot API tests, 9
  operations MCP tests, and 78 Angular tests with zero failures, errors, or
  skips, plus npm audit, Spotless, Prettier, production builds, Compose
  validation, verification-system tests, and `git diff --check`.

## Remaining limitations

- The long-lived native PostgreSQL catalog has not been populated through the
  explicit PDF command. K3 exercised the complete real corpus against
  PostgreSQL 17.11 through Testcontainers; native population, PDF embeddings,
  and live-model evaluation remain intentionally deferred to K4.

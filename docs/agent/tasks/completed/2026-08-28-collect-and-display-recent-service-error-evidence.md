...
- [x] The MCP server lists exactly one application tool named
      `getRecentServiceErrors` with the approved, stable input schema.
- [x] A known synthetic scenario returns deterministic structured service-error
      observations with source and retrieval metadata.
- [x] Available-empty, partial, not-found, unavailable, timeout, malformed, and
      invalid-input cases are distinguishable and contract-tested.
- [x] A tenant-owned investigation can create a service-error evidence
      collection attempt through the copilot API.
- [x] The copilot API records `STARTED` before tool invocation and never holds a
      database transaction open across the network call.
- [x] Every terminal result preserves the evidence ID, tool-call ID,
      investigation correlation ID, source, scenario reference, timestamps,
      schema version, and explicit status.
- [x] MCP output with mismatched identifiers, invalid status, invalid fields,
      or excessive content is stored as `MALFORMED` without fabricated
      observations.
- [x] An available response with no errors is preserved as successful negative
      evidence rather than missing evidence.
- [x] Multiple attempts are append-oriented and returned newest first; a retry
      never overwrites an earlier result.
- [x] An interrupted `STARTED` attempt remains visible and does not prevent a
      later retry.
- [x] PostgreSQL prevents evidence from being associated with an investigation
      owned by another tenant.
- [x] Cross-tenant and nonexistent investigations produce indistinguishable
      structured `404` responses and cause no tool call or persistence.
- [x] Malformed tenant and investigation identifiers produce structured `400`
      responses without a tool call or persistence.
- [x] The workspace exposes collection, retry, history, loading, empty,
      partial, unavailable, timeout, malformed, and API-error behavior without
      hiding prior attempts.
- [x] Observed evidence visibly includes provenance and is clearly separated
      from absent AI inference and recommendation.
- [x] Collection controls are pending-safe, keyboard accessible, and usable at
      390 CSS pixels without horizontal page overflow.
- [x] Existing alert intake, queue, incident detail, investigation start/resume,
      direct workspace navigation, and tenant-safe lookup behavior remain
      unchanged.
- [x] No queue, incident-detail, application-shell, existing workspace-metadata,
      or existing synthetic incident-fixture presentation change appears in the
      production diff.
- [x] Focused tests fail for the intended reason before each production
      behavior and pass afterward.
- [x] Full backend, PostgreSQL, MCP protocol, frontend, formatting, build,
      diff, and manual live verification pass with no skipped tests or new
      warnings.
- [x] The final diff contains no secrets, real payment data, generated output,
...
- 2026-08-28: Live MCP red phase discovered the tool and exact four-field input
  ...
  zero skips.
- 2026-08-28: PostgreSQL and copilot workflow tests drove Flyway V4, tenant-safe
  append-only persistence, the insert-call-update transaction boundary, strict
  MCP result validation, the synchronous Spring AI client, and the collection
  and history endpoints. Focused persistence, gateway/client, service,
  controller, and PostgreSQL API suites passed 6/6, 7/7, 4/4, 4/4, and 3/3.
- 2026-08-28: Angular API and workspace red tests failed for the absent evidence
  requests, state, action, provenance, status handling, and retry history. The
  minimum observed-evidence section then passed its 4 service and 10 workspace
  tests. The full route suite exposed and then verified a missing evidence
  history method in its route-level test double.
- 2026-08-28: The local launcher was extended to validate MCP settings and to
  start and health-check the operations MCP server before the copilot API. The
  MCP client annotation scanner was disabled because this application invokes
  the known tool directly; the focused Spring Boot/PostgreSQL test confirmed
  the new sampling and elicitation warnings were removed.
- 2026-08-28: Full automated and live verification passed. The task was marked
  complete and archived without changing the locked contract.

## Completion evidence

- Red-phase evidence: Producer compilation, fixture loading, live MCP schema and
  safety-hint checks, PostgreSQL migration/repository checks, copilot gateway
  and workflow checks, and Angular API/workspace checks each failed for the
  intended missing behavior before their production change. The final full
  Angular run also caught the incomplete route-level API test double.
- Green-phase evidence: Operations MCP passed 9/9; focused copilot persistence,
  gateway/client, workflow, controller, and live PostgreSQL API tests passed;
  the complete copilot module passed 63/63. Angular passed 38/38 after `npm ci`.
  No backend or frontend test was skipped.
- Acceptance-criteria coverage: Contract tests prove the single closed-world
  tool and every fixture outcome. PostgreSQL tests prove tenant integrity,
  append-only attempts, retained `STARTED` records, targeted completion, and no
  transaction across MCP I/O. HTTP and Angular tests prove tenant-safe APIs,
  explicit workspace states, provenance, retry history, pending safety, and
  regression behavior.
- Full verification: `mvn -pl backend/operations-mcp-server test`,
  `mvn -pl backend/copilot-api test`, `mvn clean verify`, `docker compose
  config`, `npm ci`, `npm test -- --watch=false`, `npx prettier --check .`,
  `npm run build`, `git diff --check`, and the PowerShell parser and launcher
  `--CheckOnly` checks passed. The reactor passed 72 tests and Angular passed 38,
  all with zero skips. The only JVM warnings are the pre-existing Mockito
  dynamic-agent warnings; the new MCP annotation-scanner warnings are absent.
- Manual verification: The real MCP, API, PostgreSQL, and console path produced
  `AVAILABLE` with errors, `AVAILABLE` with no errors, `PARTIAL`, `NOT_FOUND`,
  `UNAVAILABLE`, `TIMED_OUT`, and `MALFORMED`. Cross-tenant collection returned
  `404`, an invalid identifier returned `400`, and repeated collection preserved
  newest-first history. PostgreSQL retained ten synthetic verification attempts,
  including five distinct available attempts, with evidence, tool-call,
  investigation-correlation, tenant, source, schema, timestamp, and terminal
  status metadata. Desktop and 390-pixel browser checks showed provenance,
  the explicit no-AI boundary, accessible controls, every terminal state, no
  overflowing elements, and no browser warnings or errors. Queue sorting,
  detail, resume navigation, and direct workspace refresh also passed.
- Documentation updated: Safe MCP defaults, three-service launcher behavior,
  Flyway V4, current console capability, project status, and this completion
  record now match the verified implementation.
- Remaining limitations: The MCP endpoint remains unauthenticated and suitable
  only for local development. The product still has no knowledge retrieval,
  model-generated inference, report, recommendation, or human decision flow.
  Live verification intentionally added seven synthetic incidents and ten
  evidence attempts to the configured local development database.

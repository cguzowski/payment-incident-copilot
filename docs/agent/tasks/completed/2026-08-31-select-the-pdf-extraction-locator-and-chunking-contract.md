...
- [x] A new accepted ADR selects the PDF library and immutable parsed-document
      representation, with rejected alternatives and tradeoffs recorded.
- [x] The ADR defines a stable locator that can take a reviewer from a chunk to
      an exact PDF version and page, plus section/block context where reliable.
- [x] Text normalization, repeated headers/footers, page boundaries, table
      handling, empty/invalid input, and deterministic ordering are explicit.
- [x] Chunk identity, maximum/minimum sizing, overlap or non-overlap behavior,
      source-hash/parser-version/chunker-version propagation, and regeneration
      semantics are explicit.
- [x] Approval/effective/superseded behavior and tenant propagation remain
      compatible with current persistence and retrieval guardrails.
- [x] Representative extraction probes pass against an approved runbook, an
      approved policy, the densest table-oriented runbook, and a superseded
      document, with exact page locators reviewed.
- [x] The follow-on implementation task maps every contract rule to a named
      red-green test and identifies any required migration without starting it.
- [x] Repository static verification and diff checks pass.
...
- 2026-08-31: Accepted ADR-0009. It pins PDFBox 3.0.8, defines the immutable
  page/block representation, exact generated header/footer removal,
  `pdf-page-sections/v1`, lexical-before-vector persistence, and the
  migration/retrieval compatibility contract.
- 2026-08-31: PDFBox probes extracted RB-002, PL-001, RB-011, and RB-022 twice
  with identical ordered output. Page-level review located document control,
  diagnostic/table, escalation, and superseded-banner content on the expected
  physical pages without committing exploratory output.

## Completion evidence

ADR-0009 and the locked follow-on implementation test map are complete.
`./verify.ps1 -Scope Repository` passed the verification-system tests, Compose
configuration, and diff checks.

## Remaining limitations

The application still ingests only its two baseline Markdown sources. The
accepted contract is intentionally not executable until this task passes its
repository static gate and the follow-on K3 task becomes current.

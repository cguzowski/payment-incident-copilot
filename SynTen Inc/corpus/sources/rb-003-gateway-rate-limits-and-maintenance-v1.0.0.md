---
documentKey: RB-003
documentId: f6caf241-e194-4b9f-adc3-a914343f2052
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: Gateway Rate Limit and Maintenance Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Gateway capacity and maintenance responses
approvalStatus: APPROVED
approvedBy: 7b636625-53d1-46f7-92a9-9c8c27a243d1
approvedAt: 2026-08-30T08:00:00Z
effectiveAt: 2026-08-30T09:00:00Z
ownerRole: Gateway Capacity Management
classification: Internal - Synthetic Demo
replacement: None
requiredCodes: GATEWAY_MAINTENANCE_REJECTION,UPSTREAM_RATE_LIMITED
relatedDocuments: RB-002, PL-006
generatorVersion: synten-pdf-generator/v1
---
# Gateway Rate Limit and Maintenance Runbook
...
| Control | Value |
|---|---|
| Owner | Gateway Capacity Management |
...
| Related documents | RB-002, PL-006 |
| Retrieval role | Distinguish traffic limiting from planned gateway maintenance. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating gateway capacity and maintenance responses. Its operational purpose is: Distinguish traffic limiting from planned gateway maintenance. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `GATEWAY_MAINTENANCE_REJECTION` | The selected synthetic gateway route was rejecting authorizations during a maintenance window. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `UPSTREAM_RATE_LIMITED` | The upstream gateway rate-limited the authorization traffic burst. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S003 | HIGH | UPSTREAM_RATE_LIMITED | The upstream gateway rate-limited the authorization traffic burst. Confirm the stated required evidence before using this as probable cause. |
| S009 | MEDIUM | GATEWAY_MAINTENANCE_REJECTION | The selected synthetic gateway route was rejecting authorizations during a maintenance window. Confirm the stated required evidence before using this as probable cause. |
...
Escalate to Gateway Capacity Management with the incident and investigation identifiers, bounded UTC window, service name, exact error codes and counts, affected opaque route or cohort, evidence status, selected knowledge references, contradictions, and the next requested human confirmation.
...
Use RB-002, PL-006 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

---
documentKey: RB-004
documentId: 12c5326e-28b7-4a77-bdea-50836bbdc9ae
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: Authorization State Database Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Database pools, locks, and state operations
...
replacement: None
requiredCodes: AUTHORIZATION_STATE_LOOKUP_TIMEOUT,AUTHORIZATION_STATE_UPDATE_FAILED,DATABASE_CONNECTION_POOL_EXHAUSTED,DATABASE_LOCK_TIMEOUT
relatedDocuments: RB-007, RB-013, PL-002
generatorVersion: synten-pdf-generator/v1
---
# Authorization State Database Runbook
...
| Related documents | RB-007, RB-013, PL-002 |
| Retrieval role | Correlate pool exhaustion or lock contention with state lookup/update failures. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating database pools, locks, and state operations. Its operational purpose is: Correlate pool exhaustion or lock contention with state lookup/update failures. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `AUTHORIZATION_STATE_LOOKUP_TIMEOUT` | The authorization service exhausted its database connection pool, causing state lookups to time out. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `AUTHORIZATION_STATE_UPDATE_FAILED` | Database lock contention caused authorization state updates to time out. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `DATABASE_CONNECTION_POOL_EXHAUSTED` | The authorization service exhausted its database connection pool, causing state lookups to time out. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `DATABASE_LOCK_TIMEOUT` | Database lock contention caused authorization state updates to time out. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S004 | CRITICAL | DATABASE_CONNECTION_POOL_EXHAUSTED, AUTHORIZATION_STATE_LOOKUP_TIMEOUT | The authorization service exhausted its database connection pool, causing state lookups to time out. Confirm the stated required evidence before using this as probable cause. |
| S108 | HIGH | DATABASE_LOCK_TIMEOUT, AUTHORIZATION_STATE_UPDATE_FAILED | Database lock contention caused authorization state updates to time out. Confirm the stated required evidence before using this as probable cause. |
...
Use RB-007, RB-013, PL-002 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

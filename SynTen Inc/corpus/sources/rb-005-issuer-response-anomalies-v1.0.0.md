---
documentKey: RB-005
documentId: 0562dfb8-5d09-4cbe-a8d3-a27d21fdf238
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: Issuer Response Anomaly Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Issuer response categories and malformed responses
approvalStatus: APPROVED
approvedBy: 7b636625-53d1-46f7-92a9-9c8c27a243d1
approvedAt: 2026-08-30T08:00:00Z
effectiveAt: 2026-08-30T09:00:00Z
ownerRole: Issuer Integration
classification: Internal - Synthetic Demo
replacement: None
requiredCodes: ISSUER_DO_NOT_HONOR_SURGE,MALFORMED_ISSUER_RESPONSE,RESPONSE_MAPPING_FAILED
relatedDocuments: RB-017, RB-019, PL-006
generatorVersion: synten-pdf-generator/v1
---
# Issuer Response Anomaly Runbook
...
| Related documents | RB-017, RB-019, PL-006 |
| Retrieval role | Separate issuer-originated declines from transport or mapping failures. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating issuer response categories and malformed responses. Its operational purpose is: Separate issuer-originated declines from transport or mapping failures. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `ISSUER_DO_NOT_HONOR_SURGE` | Issuer-originated do-not-honor responses increased across the observed issuer group. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `MALFORMED_ISSUER_RESPONSE` | The issuer connection returned malformed synthetic responses that could not be mapped. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `RESPONSE_MAPPING_FAILED` | The issuer connection returned malformed synthetic responses that could not be mapped. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S005 | HIGH | ISSUER_DO_NOT_HONOR_SURGE | Issuer-originated do-not-honor responses increased across the observed issuer group. Confirm the stated required evidence before using this as probable cause. |
| S110 | HIGH | MALFORMED_ISSUER_RESPONSE, RESPONSE_MAPPING_FAILED | The issuer connection returned malformed synthetic responses that could not be mapped. Confirm the stated required evidence before using this as probable cause. |
...
Escalate to Issuer Integration with the incident and investigation identifiers, bounded UTC window, service name, exact error codes and counts, affected opaque route or cohort, evidence status, selected knowledge references, contradictions, and the next requested human confirmation.
...
Use RB-017, RB-019, PL-006 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

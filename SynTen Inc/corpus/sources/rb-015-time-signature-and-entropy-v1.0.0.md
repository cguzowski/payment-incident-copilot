---
documentKey: RB-015
documentId: b87eab70-680c-4336-9b63-bbb6441dd3aa
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: Time, Request Signature, and Entropy Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Clock skew, signing deadlines, and secure randomness
...
replacement: None
requiredCodes: CLOCK_SKEW_REJECTED,REQUEST_SIGNATURE_INVALID,REQUEST_SIGNATURE_TIMEOUT,SECURE_RANDOM_SOURCE_BLOCKED
relatedDocuments: RB-016, PL-005
generatorVersion: synten-pdf-generator/v1
---
# Time, Request Signature, and Entropy Runbook
...
| Related documents | RB-016, PL-005 |
| Retrieval role | Correlate timestamp, signature, or entropy signals while preserving security controls. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating clock skew, signing deadlines, and secure randomness. Its operational purpose is: Correlate timestamp, signature, or entropy signals while preserving security controls. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `CLOCK_SKEW_REJECTED` | Clock skew in one synthetic authorization instance group invalidated timestamp-bound gateway signatures. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `REQUEST_SIGNATURE_INVALID` | Clock skew in one synthetic authorization instance group invalidated timestamp-bound gateway signatures. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `REQUEST_SIGNATURE_TIMEOUT` | The isolated runtime group's secure random source blocked, delaying request signatures beyond their deadlines. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `SECURE_RANDOM_SOURCE_BLOCKED` | The isolated runtime group's secure random source blocked, delaying request signatures beyond their deadlines. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S103 | HIGH | CLOCK_SKEW_REJECTED, REQUEST_SIGNATURE_INVALID | Clock skew in one synthetic authorization instance group invalidated timestamp-bound gateway signatures. Confirm the stated required evidence before using this as probable cause. |
| S206 | CRITICAL | SECURE_RANDOM_SOURCE_BLOCKED, REQUEST_SIGNATURE_TIMEOUT | The isolated runtime group's secure random source blocked, delaying request signatures beyond their deadlines. Confirm the stated required evidence before using this as probable cause. |
...
Use RB-016, PL-005 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

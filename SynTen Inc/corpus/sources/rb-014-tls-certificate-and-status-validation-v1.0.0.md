---
documentKey: RB-014
documentId: aae2e3b7-4245-4197-a062-c7d5e7e2d978
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: TLS Certificate and Status Validation Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: TLS handshake, expiry, and OCSP validation
...
replacement: None
requiredCodes: CERTIFICATE_STATUS_CHECK_FAILED,OCSP_RESPONDER_UNAVAILABLE,TLS_CERTIFICATE_EXPIRED,TLS_HANDSHAKE_FAILED
relatedDocuments: RB-011, RB-012, PL-005
generatorVersion: synten-pdf-generator/v1
---
# TLS Certificate and Status Validation Runbook
...
| Related documents | RB-011, RB-012, PL-005 |
| Retrieval role | Separate certificate expiry from status-responder failure without bypassing checks. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating tls handshake, expiry, and ocsp validation. Its operational purpose is: Separate certificate expiry from status-responder failure without bypassing checks. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `CERTIFICATE_STATUS_CHECK_FAILED` | The certificate status responder was unavailable, so the synthetic channel failed closed during certificate validation. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `OCSP_RESPONDER_UNAVAILABLE` | The certificate status responder was unavailable, so the synthetic channel failed closed during certificate validation. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `TLS_CERTIFICATE_EXPIRED` | The synthetic gateway channel presented an expired TLS certificate, causing every handshake to fail. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `TLS_HANDSHAKE_FAILED` | The synthetic gateway channel presented an expired TLS certificate, causing every handshake to fail. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S101 | CRITICAL | TLS_CERTIFICATE_EXPIRED, TLS_HANDSHAKE_FAILED | The synthetic gateway channel presented an expired TLS certificate, causing every handshake to fail. Confirm the stated required evidence before using this as probable cause. |
| S203 | HIGH | OCSP_RESPONDER_UNAVAILABLE, CERTIFICATE_STATUS_CHECK_FAILED | The certificate status responder was unavailable, so the synthetic channel failed closed during certificate validation. Confirm the stated required evidence before using this as probable cause. |
...
Escalate to Platform Security with the incident and investigation identifiers, bounded UTC window, service name, exact error codes and counts, affected opaque route or cohort, evidence status, selected knowledge references, contradictions, and the next requested human confirmation.
...
Use RB-011, RB-012, PL-005 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

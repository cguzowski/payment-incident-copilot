---
documentKey: RB-019
documentId: 16b2705a-33f8-44c6-88f5-4aae0551891d
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: BIN and Issuer Reference Routing Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Synthetic BIN tables and issuer route lookup
approvalStatus: APPROVED
approvedBy: 7b636625-53d1-46f7-92a9-9c8c27a243d1
approvedAt: 2026-08-30T08:00:00Z
effectiveAt: 2026-08-30T09:00:00Z
ownerRole: Issuer Reference Data
classification: Internal - Synthetic Demo
replacement: None
requiredCodes: BIN_ROUTE_TABLE_STALE,ISSUER_ROUTE_NOT_FOUND
relatedDocuments: RB-005, RB-008, PL-004
generatorVersion: synten-pdf-generator/v1
---
# BIN and Issuer Reference Routing Runbook
...
| Related documents | RB-005, RB-008, PL-004 |
| Retrieval role | Verify reference-data version and refresh evidence before route attribution. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating synthetic bin tables and issuer route lookup. Its operational purpose is: Verify reference-data version and refresh evidence before route attribution. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `BIN_ROUTE_TABLE_STALE` | A stale synthetic BIN routing table left the affected issuer range without a route. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `ISSUER_ROUTE_NOT_FOUND` | A stale synthetic BIN routing table left the affected issuer range without a route. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S109 | HIGH | BIN_ROUTE_TABLE_STALE, ISSUER_ROUTE_NOT_FOUND | A stale synthetic BIN routing table left the affected issuer range without a route. Confirm the stated required evidence before using this as probable cause. |
...
Escalate to Issuer Reference Data with the incident and investigation identifiers, bounded UTC window, service name, exact error codes and counts, affected opaque route or cohort, evidence status, selected knowledge references, contradictions, and the next requested human confirmation.
...
Use RB-005, RB-008, PL-004 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

---
documentKey: RB-008
documentId: 3896ec1d-e07a-40a8-8bf3-feb5b26a6408
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: Route Configuration and Feature-Flag Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Route selection, configuration versions, and cohorts
approvalStatus: APPROVED
approvedBy: 7b636625-53d1-46f7-92a9-9c8c27a243d1
approvedAt: 2026-08-30T08:00:00Z
effectiveAt: 2026-08-30T09:00:00Z
ownerRole: Payment Routing
classification: Internal - Synthetic Demo
replacement: None
requiredCodes: CONFIG_VERSION_SPLIT_BRAIN,FEATURE_FLAG_ROUTE_MISMATCH,ROUTE_CONFIGURATION_NOT_FOUND,ROUTE_DECISION_DIVERGENCE
relatedDocuments: RB-018, RB-019, PL-004
generatorVersion: synten-pdf-generator/v1
---
# Route Configuration and Feature-Flag Runbook
...
| Related documents | RB-018, RB-019, PL-004 |
| Retrieval role | Compare approved route configuration, flag assignment, and instance decisions. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating route selection, configuration versions, and cohorts. Its operational purpose is: Compare approved route configuration, flag assignment, and instance decisions. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
| `FEATURE_FLAG_ROUTE_MISMATCH` | A synthetic feature-flag assignment selected a route configuration incompatible with the test cohort. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `ROUTE_CONFIGURATION_NOT_FOUND` | The affected merchant segment had no matching authorization route configuration. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S008 | HIGH | ROUTE_CONFIGURATION_NOT_FOUND | The affected merchant segment had no matching authorization route configuration. Confirm the stated required evidence before using this as probable cause. |
| S105 | HIGH | FEATURE_FLAG_ROUTE_MISMATCH | A synthetic feature-flag assignment selected a route configuration incompatible with the test cohort. Confirm the stated required evidence before using this as probable cause. |
...
Escalate to Payment Routing with the incident and investigation identifiers, bounded UTC window, service name, exact error codes and counts, affected opaque route or cohort, evidence status, selected knowledge references, contradictions, and the next requested human confirmation.
...
Use RB-018, RB-019, PL-004 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

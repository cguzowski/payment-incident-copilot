---
documentKey: RB-012
documentId: 66c791ca-0b67-467e-a5b0-edfe5cd3335e
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: Gateway Credential Rejection Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Opaque gateway connection credentials
...
replacement: None
requiredCodes: GATEWAY_CREDENTIAL_REJECTED
relatedDocuments: RB-014, PL-005
generatorVersion: synten-pdf-generator/v1
---
# Gateway Credential Rejection Runbook
...
| Related documents | RB-014, PL-005 |
| Retrieval role | Escalate credential state without exposing or rotating secrets. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating opaque gateway connection credentials. Its operational purpose is: Escalate credential state without exposing or rotating secrets. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `GATEWAY_CREDENTIAL_REJECTED` | The gateway rejected the synthetic service credential used by the affected connection. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S013 | CRITICAL | GATEWAY_CREDENTIAL_REJECTED | The gateway rejected the synthetic service credential used by the affected connection. Confirm the stated required evidence before using this as probable cause. |
...
Use RB-014, PL-005 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

---
documentKey: RB-017
documentId: 92d42eda-924f-4532-a4e0-ea54ca16f839
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: Gateway and Issuer Response Mapping Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Response schemas, malformed payload categories, and mappings
...
replacement: None
requiredCodes: GATEWAY_RESPONSE_SCHEMA_MISMATCH,MALFORMED_ISSUER_RESPONSE,RESPONSE_MAPPING_FAILED
relatedDocuments: RB-005, PL-005
generatorVersion: synten-pdf-generator/v1
---
# Gateway and Issuer Response Mapping Runbook
...
| Related documents | RB-005, PL-005 |
| Retrieval role | Distinguish upstream schema change from local mapping failure. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating response schemas, malformed payload categories, and mappings. Its operational purpose is: Distinguish upstream schema change from local mapping failure. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `GATEWAY_RESPONSE_SCHEMA_MISMATCH` | The gateway response schema changed incompatibly and the authorization mapper rejected the new payload shape. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
| `RESPONSE_MAPPING_FAILED` | The gateway response schema changed incompatibly and the authorization mapper rejected the new payload shape. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S106 | CRITICAL | GATEWAY_RESPONSE_SCHEMA_MISMATCH, RESPONSE_MAPPING_FAILED | The gateway response schema changed incompatibly and the authorization mapper rejected the new payload shape. Confirm the stated required evidence before using this as probable cause. |
...
Use RB-005, PL-005 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.

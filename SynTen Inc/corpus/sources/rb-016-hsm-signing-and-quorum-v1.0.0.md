---
documentKey: RB-016
documentId: 3bba9f8d-9cce-4e8f-b37a-bc0b44f7aee8
tenantId: 8b860d80-d17f-4e6b-8c48-af35f26a4d61
type: RUNBOOK
title: HSM Signing, Quorum, and Protocol Runbook
version: 1.0.0
incidentFamily: AUTHORIZATION_DECLINE_RATE_SPIKE
appliesTo: Synthetic HSM latency, quorum, and failover
approvalStatus: APPROVED
approvedBy: 7b636625-53d1-46f7-92a9-9c8c27a243d1
approvedAt: 2026-08-30T08:00:00Z
effectiveAt: 2026-08-30T09:00:00Z
ownerRole: Cryptographic Services
classification: Internal - Synthetic Demo
replacement: None
requiredCodes: HSM_FIRMWARE_PROTOCOL_MISMATCH,HSM_QUORUM_LOST,HSM_SIGNING_FAILED,HSM_SIGNING_TIMEOUT,REQUEST_DEADLINE_EXCEEDED
relatedDocuments: RB-015, PL-005
generatorVersion: synten-pdf-generator/v1
---
# HSM Signing, Quorum, and Protocol Runbook
...
| Related documents | RB-015, PL-005 |
| Retrieval role | Diagnose signing dependencies without automated failover, restart, or firmware action. |
| Classification | Internal - Synthetic Demo |
...
This runbook guides a human analyst investigating synthetic hsm latency, quorum, and failover. Its operational purpose is: Diagnose signing dependencies without automated failover, restart, or firmware action. It applies only to persisted SynTen Inc synthetic evidence for the current tenant and investigation.
...
|---|---|---|
| `HSM_FIRMWARE_PROTOCOL_MISMATCH` | The failover HSM partition used an incompatible synthetic firmware protocol and rejected signing requests. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `HSM_QUORUM_LOST` | The synthetic HSM cluster lost partition quorum and could not complete authorization signing operations. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `HSM_SIGNING_FAILED` | The failover HSM partition used an incompatible synthetic firmware protocol and rejected signing requests. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `HSM_SIGNING_TIMEOUT` | Synthetic HSM signing latency caused authorization request deadlines to expire. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
| `REQUEST_DEADLINE_EXCEEDED` | Synthetic HSM signing latency caused authorization request deadlines to expire. | Treat as observed only when the persisted window contains the code; the cause remains an inference. |
...
|---|---|---|---|
| S104 | CRITICAL | HSM_SIGNING_TIMEOUT, REQUEST_DEADLINE_EXCEEDED | Synthetic HSM signing latency caused authorization request deadlines to expire. Confirm the stated required evidence before using this as probable cause. |
| S204 | CRITICAL | HSM_QUORUM_LOST, HSM_SIGNING_TIMEOUT | The synthetic HSM cluster lost partition quorum and could not complete authorization signing operations. Confirm the stated required evidence before using this as probable cause. |
| S209 | CRITICAL | HSM_FIRMWARE_PROTOCOL_MISMATCH, HSM_SIGNING_FAILED | The failover HSM partition used an incompatible synthetic firmware protocol and rejected signing requests. Confirm the stated required evidence before using this as probable cause. |
...
Escalate to Cryptographic Services with the incident and investigation identifiers, bounded UTC window, service name, exact error codes and counts, affected opaque route or cohort, evidence status, selected knowledge references, contradictions, and the next requested human confirmation.
...
Use RB-015, PL-005 for adjacent diagnostic or governance requirements. Only approved and effective versions are retrieval eligible.
